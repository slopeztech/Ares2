...
| Password    | Random 12-char alphanumeric — generated at first boot |
| IP          | `192.168.4.1`                                        |
| Port        | `80` (HTTP/1.1)                                      |
| Max clients | 4 simultaneous                                       |

> **First-boot provisioning.** On the first power-on the ESP32 generates a random WPA2 password and a random API bearer token via `esp_random()` (hardware TRNG) and prints both to the USB-CDC console. Persist them immediately — they are only shown once.
...
The API supports a bearer token for endpoint-level access control. The token is configured via `PUT /api/device/config` and persisted on LittleFS.

### Token mode (default from first boot)

A unique API token is generated by `provisionIfFirstBoot()` during the first power-on and printed to USB-CDC. Every request to a protected endpoint must include:
...
### Open mode (legacy / explicitly cleared token)

If `api_token` is explicitly cleared via `PUT /api/device/config` the API falls back to open mode — no header required. This is provided for backward compatibility with scripts written before the security layer was added.
...
Endpoints subject to the flight lock: `PUT /api/config`, `POST /api/mode`, `POST /api/mission/activate`, `POST /api/mission/deactivate`, `POST /api/scans/i2c`, `POST /api/scans/uart`, and log downloads (with an exception when AMS status is `complete` — see below).

Scan endpoints (`POST /api/scans/i2c`, `POST /api/scans/uart`) are blocked in flight because an I2C or UART bus scan monopolises the same buses used by the barometer and IMU and could cause sensor samples to be missed during a critical flight phase.
...
**When to use:** Change the WiFi AP password, set or rotate the API bearer token, or restrict the CORS origin. Changes persist across reboots. Partial JSON is supported — include only the fields you want to change.

**Why:** On first boot, ARES auto-generates a random WPA2 password and API token (see [First-boot provisioning](#connection)). Use this endpoint if you need to change either value after provisioning.
