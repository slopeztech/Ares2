...
# Columnas mínimas para modos que sólo necesitan datos GPS.
GPS_MIN_COLUMNS = ["timestamp", "lat", "lon", "gps_alt"]


def resolve_csv_path(path):
    """Resuelve un CSV buscando en rutas comunes del proyecto."""
    if os.path.isabs(path) and os.path.isfile(path):
        return path

    # 1) Ruta relativa al cwd actual
    ...
    # Mantener el nombre original para un mensaje de error claro.
    return path
...
def load_csv(path):
    ...
    if not os.path.isfile(resolved_path):
        ...
        raise FileNotFoundError(
            ...
            f"Prueba con: '{os.path.join('scripts', base)}' o "
            f"'{os.path.join('scripts', 'downloads', base)}'. "
            f"Directorio del script: '{script_dir}'"
        )

    df = pd.read_csv(resolved_path)

    # Compatibilidad con cabecera de telemetria actual.
    header_map = {
        "t_ms": "timestamp",
        "state": "mode",
        "imu_ax": "acc_x",
    ...
    # Fallback: CSV sin cabecera en orden legacy.
    if "timestamp" not in df.columns:
        df = pd.read_csv(resolved_path, names=COLUMNS)

    missing = [c for c in COLUMNS if c not in df.columns]
    if missing:
        # Tolerar CSVs GPS-only si contienen al menos las columnas mínimas.
        gps_missing = [c for c in GPS_MIN_COLUMNS if c not in df.columns]
        if gps_missing:
            raise ValueError(
                f"CSV con columnas incompatibles. Faltan: {missing}. "
                f"Columnas detectadas: {list(df.columns)}"
            )
        # Rellenar columnas de sensor ausentes con NaN para mantener el schema.
        for c in missing:
            df[c] = float("nan")
    ...
    # Conserva columnas adicionales (por ejemplo, wind_mps) sin romper
...
def plot_gyro(df, output):
    ...
    plt.grid(True)
    plt.savefig(output)
    print(f"Gyro plot guardado en {output}")
...
def generate_summary(df, output):
    summary = {
        "duracion_s": df["time_s"].max(),
        "altitud_max": df["altitude"].max(),
        "velocidad_max": df["speed"].max(),
        "aceleracion_max": df["acc_total"].max(),
        "temperatura_media": df["imu_temp"].mean()
    }

    summary_df = pd.DataFrame([summary])
    summary_df.to_csv(output, index=False)
    print(f"Resumen guardado en {output}")
...
def generate_map(df, output):
    ...
    if gps.empty:
        print("No hay datos GPS válidos")
        return
...
def generate_map_3d(df, output):
    ...
    # Prioriza altitud GPS; si no existe, usa la barométrica.
    gps["z_alt"] = gps["gps_alt"].fillna(gps["altitude"])
    gps = gps.dropna(subset=["z_alt"])
    if gps.empty:
        print("No hay datos de altitud válidos para mapa 3D")
        return
    ...
    fig.update_layout(
        title="Trayectoria 3D (lon/lat/altitud)",
        scene={
    ...
    print(f"Mapa 3D guardado en {output}")
...
def generate_map_real(df, output, satellite=False, animation=False):
    ...
    def write_deck_animation_html(gps_df, events, out_path, use_satellite):
        ...
        # Estilo mapa para MapLibre (sin token).
        style_obj = {
            "version": 8,
            "sources": {
                "base": {
...
<head>
    ...
    <title>Mapa real 3D animado</title>
...
<body>
    <div id=\"map\"></div>
    <div id=\"hud\">Inicializando animación...</div>
    <div id=\"controls\">
    ...
    <script>
        ...
        // Mini visor 3D de actitud (cohete en esquina).
        let miniReady = false;
        let miniScene = null;
        let miniCamera = null;
        ...
        try {{
            if (!window.THREE) {{
                throw new Error('THREE no disponible');
            }}
            ...
            // Cuerpo (eje local +Y = nariz del cohete).
            ...
            console.warn('No se pudo inicializar el mini visor 3D:', err);
            if (attitudePanel) {{
        ...
        function getCurrentPose(currentTime) {{
            ...
            // Vector de orientación en metros (para evitar mezclar grados con metros).
            ...
            // Al inicio, forzar vertical para representar salida guiada.
            if (currentTime <= 2000) {{
                dir = [0, 0, 1];
            }}
        ...
        function updateAttitude(pose) {{
            ...
            // Pitch respecto al horizonte (90° vertical arriba).
            const horiz = Math.sqrt(dx * dx + dy * dy);
            const pitchDeg = Math.atan2(dz, Math.max(1e-6, horiz)) * 180 / Math.PI;

            // Orientar modelo 3D: +Y del modelo -> vector de dirección en mundo ENU.
            if (miniReady) {{
                ...
                // Giro lento del anillo para dar referencia visual de rumbo.
        ...
        function renderAt(currentTime) {{
            ...
            if (pose) {{
                ...
                rocketHeadLayer = new deck.ScatterplotLayer({{
                    ...
                    data: [{{
                        ...
                        tooltip: `<b>Cohete</b><br/>t=${{fmt(t)}}s`
                    }}],
                    getPosition: d => d.position,
                    getRadius: d => d.radius,
                    radiusMinPixels: 6,
            ...
            hud.innerHTML =
                `<b>{'Satélite' if use_satellite else 'Mapa'} 3D animado</b><br/>` +
...
"""
    ...
    # Prioriza altitud GPS; fallback a barométrica.
    gps["z_alt"] = gps["gps_alt"].fillna(gps["altitude"])
    gps = gps.dropna(subset=["z_alt"])
    if gps.empty:
        print("No hay datos de altitud válidos")
        return
    ...
    # Modo satélite y/o animación en mapa real 2D (Folium).
    # Modo por defecto (sin satélite ni animación): mantiene el 3D georreferenciado (PyDeck).
    if animation:
        ...
        print(
            ...
            f"duracion={gps['time_s'].max():.1f}s"
        )
        print(f"Mapa real guardado en {output}")
        return
    ...
    if satellite:
        ...
        # Marcadores clave con resumen rápido.
        for ev in event_rows:
            popup = (
                f"<b>{ev['name']}</b><br>"
                f"t={ev['time_s']:.2f}s<br>"
                f"alt={ev['z_agl']:.1f}m AGL<br>"
                f"vel={ev['speed']:.2f}m/s<br>"
                f"g={ev['g_force']:.2f}"
            )
            folium.CircleMarker(
        ...
        title = "Mapa real (satélite)" if satellite else "Mapa real"
        title_html = (
            "<h4 style='position: fixed; top: 8px; left: 50px; z-index: 1000; "
            "background: rgba(0,0,0,0.6); color: #fff; padding: 6px 10px; "
            "border-radius: 6px;'>"
            f"{title} | Alt max: {gps.loc[max_alt_idx, 'z_agl']:.1f} m | "
            f"g max: {gps.loc[max_g_idx, 'g_force']:.2f} g"
            "</h4>"
        )
    ...
    # Un único visualizador 3D georreferenciado (modo por defecto).
    path_coords = gps[["lon", "lat", "z_agl"]].values.tolist()
    path_data = [{"name": "Trayectoria", "path": path_coords}]

    # Muestra puntos con color por altura para facilitar lectura de subida/bajada.
    pts = gps[["lon", "lat", "z_agl", "time_s", "timestamp_s", "speed", "g_force"]].copy()
    max_agl = max(float(pts["z_agl"].max()), 1.0)

    def point_color(z):
        # Azul->rojo simple en RGB, sin dependencias extra.
...
def detect_events(df):
    ...
    if not launch.empty:
        print(f"Posible lanzamiento: t={launch.iloc[0]['time_s']:.2f}s")

    apogee = df["altitude"].idxmax()
    print(f"Apogeo: {df.iloc[apogee]['altitude']}m en t={df.iloc[apogee]['time_s']:.2f}s")
...
def main():
    ...
    parser.add_argument("csv_file", help="Ruta del CSV")
    parser.add_argument(
        "mode",
        choices=[
            "altitude_plot",
    ...
    parser.add_argument(
        "--satellite",
        action="store_true",
        help="Usar mapa base en vista satélite (solo map_real)",
    )
    parser.add_argument(
        "--animation",
        action="store_true",
        help="Activar modo animación temporal de trayectoria (solo map_real)",
    )
